...
public class TestTimeSeriesReferenceDataDAOImpl extends BaseDatastoreTestCase {
	...
	public TimeSeriesData saveTimeSeries(TimeSeriesDataDAO dao,
			double[] values, int checkNumberAfterAdd) {
	...
	public void testCreate() {
		...
		tsref1.setTimeSeriesId(new Key<TimeSeriesData>(TimeSeriesData.class,
				tsd1.getId()));
		...
		List<TimeSeriesReferenceData> list = refDao
				.findBySourceAndLocationAndType("source1", "location1", "type1");
		assertNotNull(list);
		...
		TimeSeriesData tsd3 = dao.findObjectById(TimeSeriesData.class, tsdKey3
				.getId());
