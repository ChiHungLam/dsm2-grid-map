...
Plots.prototype.xsection_editor = function(div_id,xsection_points,profile,points){
	...
	var xvals = pv.map(profile,function(d){ return d.x;});
	var yvals = pv.map(profile, function(d){return d.y;});
	var x = pv.Scale.linear(xvals).range(w2,w-w2);
	var y = pv.Scale.linear(yvals).range(h-h2,h2);
