...
public class DEMDataAppEngineCSVEncoder {
	...
	public static void main(String[] args) throws Exception {
		if (args.length != 2) {
			System.out
					.println("Usage: DEMDataAppEngineCSVEncoder <directory> <out_file>");
			System.exit(1);
		}
		...
		if (dir.exists() && dir.isDirectory()) {
			...
			System.out.println("Processing "+files.length+" in directory: "+dir);
	...
	public static String encodeFile(String file) throws Exception {
		...
		while ((line = reader.readLine()) != null) {
			...
			for (int i = 0; i < fields.length; i++) {
				dos.writeInt(Integer.parseInt(fields[i]));
			}
		}
		reader.close();
		baos.close();
		String val = base64Encoder.encode(baos.toByteArray());
		return val.replace(LINE_SEPARATOR, "");
	}
}
