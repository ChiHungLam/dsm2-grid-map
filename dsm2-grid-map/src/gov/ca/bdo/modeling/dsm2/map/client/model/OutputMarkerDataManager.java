...
public class OutputMarkerDataManager {
	...
	public void addMarkers(MapWidget map) {
		List<ChannelOutput> channelOutputs = model.getOutputs()
				.getChannelOutputs();
		for (ChannelOutput channelOutput : channelOutputs) {
			...
			LatLng position = calculateMarkerLocation(channelOutput.channelId,
					distance);
			if (position == null) {
				continue;
			}
	...
	private void addOutputMarkerToMap(MapWidget map, LatLng position,
			final ChannelOutput output) {
		final String outputName = output.name;
		if (outputVariableMap.containsKey(outputName)) {
			String newValue = outputVariableMap.get(outputName) + ","
					+ output.variable;
		...
		outputMarker.addMarkerClickHandler(new OutputClickHandler(outputName,
				this, mapPanel));
	...
	private LatLng calculateMarkerLocation(String channelId, double distance) {
		...
		double x2 = upNode.getLatitude();
		double y2 = upNode.getLongitude();
		double x1 = downNode.getLatitude();
		double y1 = downNode.getLongitude();
		double slope = (y2 - y1) / (x2 - x1);
		// double xn = Math.sqrt(distance * distance / (1 + slope * slope)) +
		// x1;
	...
	public void getRegularTimeSeries(String name, String[] variables,
			final OutputPanel panel) {
		dataService.getRegularTimeSeries(mapPanel.getCurrentStudy(), name,
				variables, new AsyncCallback<List<RegularTimeSeries>>() {
